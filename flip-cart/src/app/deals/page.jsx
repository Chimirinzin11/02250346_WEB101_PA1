...
export default function DealsPage() {
  ...
  const filtered = deals.filter((p) => {
    if (filter === "Electronics") return p.category === "Electronics";
    if (filter === "Fashion") return p.category === "Fashion";
    if (filter === "Home") return p.category === "Home";
  ...
  return (
    <main className="bg-[#f1f3f6] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* Back button */}
        <Link href="/">
          <button className="mb-4 flex items-center gap-2 text-sm text-green-600 hover:underline">
            ← Back to Home
          </button>
        </Link>
        ...
        <div className="bg-white px-4 py-3 rounded-sm shadow-sm mb-4 flex gap-3 flex-wrap">
          ...
          {["All", "Electronics", "Fashion", "Home", "Top Rated"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
